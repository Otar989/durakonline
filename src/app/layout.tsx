...
export default function RootLayout({
  ...
  <script dangerouslySetInnerHTML={{ __html: `(()=>{const send=()=>{try{const logs=(window.__durak_perf_logs||[]); if(!logs.length) return; navigator.sendBeacon? navigator.sendBeacon('/api/vitals', JSON.stringify({ logs })) : fetch('/api/vitals',{ method:'POST', body: JSON.stringify({ logs }), headers:{'content-type':'application/json'} }); window.__durak_perf_logs=[];}catch{}}; if(window.requestIdleCallback){requestIdleCallback(()=>{send(); setInterval(send,15000);});} else {setTimeout(send,4000);} window.addEventListener('beforeunload', send);})();` }} />
        <ErrorBoundary>
          <SettingsProvider>
